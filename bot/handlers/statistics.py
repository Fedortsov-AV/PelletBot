...
async def process_arrivals_period(message: Message, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        if not arrivals:
            await message.answer(f"📥 Нет данных о приходах за период {message.text}")
            return
...
async def handle_arrivals_month(callback: CallbackQuery, session: AsyncSession):
    ...
    try:
        ...
        if not arrivals:
            await callback.message.answer("📥 Нет данных о приходах за текущий месяц")
            return
...
async def handle_user_expenses(callback: CallbackQuery, session: AsyncSession):
    ...
    try:
        total = await get_user_expenses(session, callback.from_user.id)
        await callback.message.answer(
